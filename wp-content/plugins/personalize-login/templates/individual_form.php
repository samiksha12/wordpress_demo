...
function register_form( $username, $email, $full_name, $position, $descrip, $address, $oname, $oemail, $contact, $password, $auth ) {
	echo '
    <style>
    div {
        margin-bottom:2px;
    ...
    <form action="' . $_SERVER['REQUEST_URI'] . '" method="post" enctype="multipart/form-data">
        <div>
    <fieldset> 
	<legend><label class="form-label">Individual Details</label></legend>
	<div>
    <label for="oname" class="form-label">Individual Name</label>
    <input type="text" class="form-input" id="oname" name="oname" value="' . ( isset( $_POST['oname'] ) ? $oname : null ) . '">
    </div>
	<div>
    <label for="oemail" class="form-label">Individual Email Address</label>
	...
	<div>
    <label for="fname" class="form-label">POC name</label>
    <input type="text" class="form-input" id="fname" name="fname" value="' . ( isset( $_POST['fname'] ) ? $full_name : null ) . '">
    </div>
    <div>
    <label for="position" class="form-label">POC Relationship</label>
	...
	<div>
	<input type="checkbox" name="validauth" value="auth"' . ( isset( $_POST['validauth'] ) ? 'checked' : null ) . '>Click the box below if you are authorized to raise funds for the Individual listed above
	</div>
...
function register_validation( $username, $full_name, $oemail, $email, $password ) {
...
function complete_register() {
	...
	if ( 1 > count( $reg_errors->get_error_messages() ) ) {
		...
		add_user_meta( $user, 'individual', 'yes' );
